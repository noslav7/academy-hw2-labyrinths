...
- Реализация алгоритмов генерации лабиринта: поиск в глубину (DFS), алгоритм Прима. Дополнительно поддерживается алгоритм Краскала.
- Реализация алгоритмов поиска пути из заданной начальной точки **O** в конечную точку **X**: А* (A-star), Дейкстры. Дополнительно поддерживаются BFS (без учёта весов) и Greedy Best-First Search (жадный поиск по эвристике).
...
java -jar app.jar generate --algorithm=kruskal --width=31 --height=21 --output=maze3.txt
...
java -jar app.jar solve --algorithm=bfs      --file=maze2.txt --start=0,0 --end=9,9
java -jar app.jar solve --algorithm=greedy   --file=maze2.txt --start=0,0 --end=9,9
```

### Формат файла с лабиринтом

Обозначения ячеек лабиринта и вывода:

- **#** — стена (непроходимая ячейка)
- **Пространство ' '** — обычная поверхность (NORMAL, стоимость 1)
- **~** — песок (SAND, стоимость 3)
- **^** — болото (SWAMP, стоимость 5)
- **.** — хорошее покрытие (PAVEMENT, стоимость 0)

Обозначения в выводе результата:

- **O** — начальная точка
- **X** — конечная точка
- **+** — найденный маршрут (поверх текущей поверхности)
...
- Изучите и реализуйте алгоритмы генерации лабиринтов: Прима, DFS (дополнительно возможно: Краскала).
- Реализуйте алгоритмы поиска пути: Дейкстры и А* (дополнительно возможно: BFS, Greedy Best-First Search).
- Проверьте работоспособность программы с разными параметрами и напишите тесты.
- Разработайте способ отображения лабиринтов и путей в них, используя символьную графику. Можно использовать простые символы (#, ~, ^, ., +), можно более сложные (unicode).
