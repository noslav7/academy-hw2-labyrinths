...
public final class MazeValidator {
    ...
    /**
     * Ensures that the given start and end points are within the maze bounds and not walls.
     * Exits with status code 2 if validation fails.
     */
    ...
    private static void requireNavigablePoint(Maze maze, Point point, int width, int height, String pointName) {
        ...
        if (maze.cells()[point.y()][point.x()] == CellType.WALL) {
            System.err.println(
                    pointName + " point is on a wall: " + point + ". Choose coordinates where the maze has a space ' '.");
            System.exit(2);
        }
    }
